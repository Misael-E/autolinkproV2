...
const TopCustomers = async ({
  ...
  const map: Record<string, { name: string; jobs: number; revenue: number }> = {};
  ...
  return (
    <div className="bg-aztecBlack-dark rounded-xl p-5">
      ...
      {ranked.length === 0 ? (
        <p className="text-sm text-gray-500">No invoices this period</p>
      ) : (
        <div className="flex flex-col gap-3">
          {ranked.map((c, i) => {
            const style = rankStyles[i] ?? { badge: "bg-aztecBlack-light text-gray-400", glow: "" };
              ...
              <div key={c.name} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-bold text-aztecGreen flex-shrink-0">
                  ...
                  <div className="w-full bg-aztecBlack rounded-full h-1 overflow-hidden">
                    <div
                      className="h-1 rounded-full bg-gradient-to-r from-aztecBlue to-aztecGreen"
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
