...
const ServiceForm = ({
		...
		try {
			...
			if (pricing) {
				if (showPricingMode) {
					...
					if (pricing.glassCost != null)
						setGlassCost(pricing.glassCost.toString());
					if (pricing.flatCharge != null)
						setValue("price", pricing.flatCharge.toString());
				} else {
					// Invoice/appointment context: price = glassCost + flatCharge (final charge)
					const finalCharge =
						(pricing.glassCost ?? 0) + (pricing.flatCharge ?? 0);
					setValue("price", finalCharge.toFixed(2));
				}
				if (pricing.materialCost != null)
					setValue("materialCost", pricing.materialCost.toString());
				if (pricing.shopFees != null)
					setValue("shopFees", pricing.shopFees.toString());
				if (pricing.gasCost != null)
					setValue("gasCost", pricing.gasCost.toString());
	...
	const onSubmit = handleSubmit(async (serviceData) => {
		const effectivePrice = showPricingMode
			? finalQuotedPrice.toFixed(2)
			: serviceData.price;
		const newService = {
		...
		const isCustom = !staticServices.some(
			(s) => s.value === serviceData.serviceType,
		);
	...
	const handleServiceSelect = (
		...
		if (selectedOption) {
			...
			if (selectedOption.price != null)
				updates.price = selectedOption.price.toString();
					...
					<div className="col-span-full flex flex-col gap-1 bg-[#1a1a1a] border border-gray-700 rounded-lg px-4 py-3">
						<div className="flex items-center justify-between">
							<span className="text-xs text-gray-500">
								Cost + Flat Charge + Fees
							</span>
							<span className="text-xs text-gray-500">
								${parseFloat(glassCost || "0").toFixed(2)} + $
