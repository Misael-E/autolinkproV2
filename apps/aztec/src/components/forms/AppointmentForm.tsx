...
import { faClose, faFileContract, faPencil, faPlus } from '@fortawesome/free-solid-svg-icons';
...
const AppointmentForm = ({
  ...
  const debouncedLoadQuotes = useMemo(() => {
    return _.debounce(
      (inputValue: string, callback: (options: QuoteOptionType[]) => void) => {
        fetch(`/api/quote?search=${inputValue}&location=${locationSlug}`)
          .then((res) => res.json())
          .then((data: any[]) => {
  ...
  // Build pre-fill data for the QuoteForm from current form state
    ...
    <>
        ...
        {isMobile && showServiceModal ? (
          ...
          <div className="flex flex-col md:flex-row gap-4 md:gap-6">
            ...
            <div className="flex flex-col gap-6 md:w-1/2">
              {selectedQuote && (
                <div className="flex items-center gap-2 text-xs text-aztecBlue bg-aztecBlue/10 border border-aztecBlue/30 rounded-lg px-3 py-2">
                  <FontAwesomeIcon icon={faFileContract} className="w-3.5" />
      ...
      {showQuoteModal && (
        <div className="w-screen h-screen fixed left-0 top-0 bg-black bg-opacity-80 z-50 flex items-center justify-center">
          <div className="bg-aztecBlack-dark border border-gray-700/50 p-6 rounded-xl relative w-[90%] md:w-[70%] lg:w-[60%] xl:w-[70%] max-h-[90vh] overflow-y-auto">
            <QuoteForm
              type="create"
