  ...
  return (
    <div className="bg-aztecBlack-dark rounded-xl p-5 h-full flex flex-col">
      <div className="flex justify-between items-start mb-5">
        ...
        <div className="flex items-center gap-1 bg-aztecBlack p-1 rounded-full">
          <Link
            href={{
          ...
          >
            <span
              className={`px-3 py-1 rounded-full text-xs font-medium block transition-all ${
                sortBy === "revenue" ? "bg-aztecBlue text-white" : "text-gray-400 hover:text-white"
              }`}
            >
              Revenue
            </span>
          ...
          >
            <span
              className={`px-3 py-1 rounded-full text-xs font-medium block transition-all ${
                sortBy === "jobs" ? "bg-aztecBlue text-white" : "text-gray-400 hover:text-white"
              }`}
            >
              Jobs
            </span>
      ...
      {raw.length === 0 ? (
        <p className="text-sm text-gray-500">No services for this period</p>
      ) : (
          ...
          {raw.map((item, i) => {
            ...
            return (
              <div
                ...
                className={`flex items-center gap-3 p-3 rounded-lg bg-aztecBlack-light ${isTop3 ? `border-l-2 ${rankBorderColors[i]}` : ""}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-1.5">
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      <span className={`text-xs font-semibold ${sortBy === "jobs" ? "text-aztecBlue" : "text-gray-500"}`}>
                        {jobs} job{jobs !== 1 ? "s" : ""}
                      </span>
                      <span className={`text-sm font-bold ${sortBy === "revenue" ? "text-aztecGreen" : "text-gray-500"}`}>
                        ${revenue.toLocaleString()}
                      </span>
                    </div>
                  </div>
                  <div className="w-full bg-aztecBlack rounded-full h-1.5 overflow-hidden">
                    <div
                      className={`h-1.5 rounded-full transition-all ${
                        sortBy === "jobs"
                          ? "bg-gradient-to-r from-aztecBlue to-aztecBlue/60"
                          : "bg-gradient-to-r from-aztecBlue to-aztecGreen"
                      }`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
