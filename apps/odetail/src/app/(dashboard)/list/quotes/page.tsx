...
const QuoteListPage = async ({
    ...
    >
      <td className="flex items-center gap-4 p-4">
        <div className="flex flex-col">
          ...
          <a href={`tel:${item.customer.phone}`} className="text-xs text-odetailBlue lg:hidden">
            {formatPhoneNumber(item.customer.phone)}
          </a>
          <p className="text-xs text-gray-300">#{item.quoteNumber ?? item.id}</p>
        </div>
      </td>
  ...
  const query: Prisma.QuoteWhereInput = { companyId: "odetail" };
