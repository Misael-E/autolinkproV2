...
const CustomerListPage = async ({
  ...
  const renderRow = (item: Customer) => (
    <tr
      key={item.id}
      className="border-b border-gray-200 even:bg-odetailBlack-light text-sm text-white hover:bg-odetailBlue"
    >
        ...
        switch (key) {
          case "search":
            query.OR = [
              ...
              { customerType: { contains: value, mode: "insensitive" } },
            ];
            break;
          default:
            break;
    ...
    <div className="flex m-4 gap-4 flex-col">
      <div className="flex items-center gap-4 px-4 text-sm font-semibold justify-between">
        <div className="flex items-center gap-4 self-end">
          ...
          >
            <button
              className={`p-2 rounded ${dateRange === "allTime" ? "bg-odetailBlue text-white" : "bg-gray-200 text-black"}`}
            >
              All Time
            </button>
          </Link>
