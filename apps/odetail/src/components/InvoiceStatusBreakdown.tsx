...
> = {
  Paid: {
    bar: "bg-odetailGreen",
    text: "text-odetailGreen",
    bg: "bg-odetailGreen/10",
    dot: "bg-odetailGreen",
  },
  Pending: {
    bar: "bg-odetailOrange",
    text: "text-odetailOrange",
...
const InvoiceStatusBreakdown = async ({
  ...
  for (const inv of revenueByStatus) {
    ...
    revenueMap.set(inv.status, (revenueMap.get(inv.status) ?? 0) + total);
  }
    ...
    <div className="bg-odetailBlack-dark rounded-xl p-5 h-full flex flex-col">
      ...
      <div className="flex flex-col gap-4 flex-1">
        {sorted.map((g) => {
          ...
          const revenue = revenueMap.get(g.status) ?? 0;
          ...
          return (
            <div key={g.status}>
              ...
              <div className="w-full bg-odetailBlack rounded-full h-2 overflow-hidden">
                <div
      ...
      <div className="mt-5 pt-4 border-t border-odetailBlack-light grid grid-cols-2 gap-3">
        {sorted.map((g) => {
          const cfg = statusConfig[g.status] ?? statusConfig["Draft"]!;
          const revenue = revenueMap.get(g.status) ?? 0;
          return (
