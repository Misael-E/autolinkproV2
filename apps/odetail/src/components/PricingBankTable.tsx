...
import PricingBankNewEntryRow from "./PricingBankNewEntryRow";
...
const inputCls =
	"bg-gray-700 text-white text-sm rounded px-2 py-0.5 border border-odetailBlue focus:outline-none";
...
export default function PricingBankTable({
		...
		try {
			...
			if (field === "glassCost" || field === "flatCharge") {
				...
				if (res.ok) {
					...
					setEntries((prev) =>
						prev.map((e) =>
							entryKey(e) === key ? { ...e, [field]: numVal } : e,
						),
					);
					setEditingCell(null);
				}
	...
	const deleteEntry = (entry: PricingEntry) => {
		if (!entry.id) return;
		const toastId = toast(
				...
				<div className="flex items-start gap-3">
					...
					<div>
						...
						<p className="text-xs text-gray-400 mt-0.5">
							This will permanently remove this entry from the bank.
						</p>
						<div className="flex items-center gap-2 mt-1.5">
							<span className="font-mono text-white bg-gray-700 px-1.5 py-0.5 rounded text-xs">
								{entry.code}
							</span>
							{entry.distributor && (
								<span className="text-xs text-gray-300 bg-gray-700/60 border border-gray-600 px-1.5 py-0.5 rounded">
									{entry.distributor}
								</span>
							)}
							<span
								className={`text-xs px-1.5 py-0.5 rounded ${categoryColors[entry.customerType] ?? categoryColors.Other}`}>
								{entry.customerType}
							</span>
						</div>
					</div>
						...
						onClick={async () => {
							...
							if (res.ok) {
								...
								toast.success("Pricing entry deleted.", {
									style: { background: "#1a1f2e", border: "1px solid #1194e4", borderRadius: "0.75rem" },
									progressStyle: { background: "#1194e4" },
								});
							} else {
								toast.error("Failed to delete entry.", {
									style: { background: "#1a1f2e", border: "1px solid #ef4444", borderRadius: "0.75rem" },
									progressStyle: { background: "#ef4444" },
								});
							}
						}}
						className="flex-1 py-1.5 bg-red-500 hover:bg-red-600 active:bg-red-700 text-white text-xs font-medium rounded transition-colors">
						Delete
					</button>
					<button
						onClick={() => toast.dismiss(toastId)}
						className="flex-1 py-1.5 bg-gray-700 hover:bg-gray-600 active:bg-gray-500 text-gray-200 text-xs font-medium rounded transition-colors">
						Cancel
					</button>
				</div>
			</div>,
	...
	return (
		<div className="overflow-x-auto">
			<div className="flex justify-end px-5 py-3 border-b border-gray-700/60">
				{showNewRow ? (
					<button
					...
					<button
						...
						className="flex items-center gap-1.5 text-xs font-medium text-odetailBlue hover:text-blue-300 transition-colors">
			...
			<table className="w-full text-sm">
				...
				<tbody className="divide-y divide-gray-700/40">
					...
					{entries.length === 0 && !showNewRow && (
						<tr>
							<td
								...
								No pricing data yet. Click{" "}
								<span className="text-odetailBlue">Add Entry</span> to create
								one manually.
							</td>
						</tr>
					)}
					{entries.map((entry, i) => {
						...
						const marginVal =
							finalPrice > 0 ? (entry.flatCharge / finalPrice) * 100 : 0;
						const marginDisplay =
						...
						return (
							<tr key={i} className="hover:bg-gray-800/50 transition-colors">
								...
								<td className="px-5 py-3.5">
									{editingCategory ? (
										...
										<div className="flex items-center gap-1.5 group/category">
											<span
												className={`px-2.5 py-1 rounded-full text-xs font-medium ${catStyle}`}>
												{entry.customerType}
											</span>
											<button
												onClick={() =>
													startEdit(entry, "customerType", entry.customerType)
												}
								...
								<td className="px-5 py-3.5 text-gray-500 text-xs">
									<div className="flex items-center gap-2 group/updated">
										<span>
										...
										{entry.id != null && (
											<button
												...
												className="opacity-0 group-hover/updated:opacity-100 text-gray-600 hover:text-red-400 transition-opacity"
												title="Delete entry">
